...
- **新「日志」导航页**：双 tab（dsh-web.out.log / dsh-web.err.log）tail 查看控制台拉起的
  dsh web 落盘输出（`%TEMP%\dsh-dash`，start_dsh 既有重定向，此前无任何界面可见）——
  cordis loader 警告、Node 堆栈、启动 URL 等首次有了查看入口
...
- 纯单元 335 例全绿（+19 例）；离屏单页构造冒烟真机日志通过
