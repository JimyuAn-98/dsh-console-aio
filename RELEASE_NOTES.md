...
- **隧道卡误报绿灯**：卡片绿灯此前仅依据被监视端口是否在监听（`card_states_from_monitor`），端口被其它进程占用时会"没运行也绿灯"。改为以**隧道进程存活**为权威（`TunnelManager.running_map` 读 `tunnel-pids.json` + 探活）；反向隧道要求进程存活且远端端口在听。
