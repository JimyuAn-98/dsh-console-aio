...
- 🚀 一键操作：本机 dsh 启停、三条 SSH 隧道（启动/常驻/停止）、dsh 一键更新
...
## 界面布局（示意）
    ...
    顶部:  [ dsh 控制台 ]    本机轮询 4s · SSH直查 20s    [配置] [立即刷新]
...
所有可调项都在同目录的 config.json，也可点界面右上角 【配置】 编辑常用项（保存后重启生效）。
...
| 字段 | 说明 | 默认 |
|------|------|------|
| ssh_server | 公网中转服务器 IP | 185.238.250.148 |
| ssh_user | 中转服务器隧道用户 | tunnel |
| dash_repo | 本机 dsh 仓库路径 | D:\Applications\deepseek-harness |
