# -*- coding: utf-8 -*-
# test_core.py - core 业务层纯单元测试(默认 pytest 层, 零真实资源)。
#
...
from core.config import derived, load_config, load_derived
...
class TestCoreDataExports:
    # 数据层统一入口 core.data: 私有 YAML/SSH 助手与公共符号均从该模块导出
    # (原仓库根 dsh_data.py 兼容 shim 已删除, 目录整理时收敛到 core.data)。
    def test_core_data_binds_same_module_objects(self):
    ...
    def test_core_data_exports_private_helpers(self):
        # tests/ 与历史调用方引用过私有 YAML/SSH 助手, core.data 必须导出
        from core import data as dsh_data
