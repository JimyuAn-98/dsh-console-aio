# -*- coding: utf-8 -*-
# SSH 密钥管理页(UI 层): 只做展示/确认框/busy 管理, 业务在 core/keys.py(纯 Python)。
...
# 公钥(.pub)为公开信息, 经 core.keys.read_pubkey 同步直读展示/复制。
...
class KeysPage(BasePage):
    ...
    def _build(self):
        ...
        head = QHBoxLayout()
        head.addWidget(QLabel("SSH 密钥管理", objectName="cardTitle"))
        head.addStretch(1)
