...
| 2 | DSH 管理 | dsh | ✅ 已核对 | 批1 版本发布日志查看(含状态色 token 化); 批2 部署指定版本/固定/回退 + 更新/部署进度条; 批3 长操作完整输出日志(「打开操作日志」按钮, 落盘前 Token 脱敏) + 卸载删除加固; 批4 双安装模式(源码/全局包)自动检测 + 按模式分流启停/更新/卸载 + 安装方式选择 |
