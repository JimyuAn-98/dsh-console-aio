...
- ✅ **dsh 启动控制台报错捕获、插件管理 Profile 死锁修复与插件更新能力接入（2026-09-10）**：
  - `core/dshctl.py` 引入增量 tail 机制与启动前端口预检：拉起新进程前主动探测目标端口并清理旧残留进程，防止端口被占导致的秒级假就绪；
  - 启动子进程后进行 15s 初始存活与错误流式观测，并配合后台持续 15s 异步监视器（去除 Token 提前退出限制），若进程因插件异步加载冲突（如 SyntaxError）崩溃，直接将 `dsh-web.err.log` 堆栈打入底部控制台日志区并标记状态卡片离线；
  - 强化 `stop_dsh`：主动定位并 kill 监听端口 PID，扩大 Node 进程匹配范围，确保端口干净释放；
  - `update_dsh` 校验重启步骤返回值，失败时中止并设置状态为“更新完成但启动失败”；
  - 修复 `run_dsh("restart")` 历史时序笔误（先 stop 后 start）；托盘菜单补充 `stop_dsh` / `restart_dsh` 对应调用；
  - 修复 `ui/pages_plugins.py`：在 `_apply_profiles` 中解开 busy 状态，彻底解决首部防重拦截导致的界面永久死锁卡在“正在读取 Profile 列表...”；
  - 插件管理页扩充【更新】按钮（更新选中插件）与【更新全部插件】按钮（一键批量更新当前 Profile 所有插件），优化【安装】按钮支持弹窗输入任意 npm 包名/本地路径/Git 链接；
  - 新增 10 个相关单元测试，全量 435 单元测试 + 86 GUI 冒烟测试全过。

- ✅ **SSH 隧道生命周期治理、动态监控修复与自检重构（2026-09-03）**：
  - 确立单一事实源：废除历史静态端口覆盖，右侧健康监控与后台探测端口完全由当前方案的 `tunnels` 动态生成；
  - 补齐进程生命周期：应用方案时自动终止上一方案所有运行中的旧隧道，彻底根除端口冲突与 Windows 孤儿僵尸进程；
  - 根治方案下拉框内存陈旧缓存，CRUD 后即时刷新方案与卡片；
  - 重构启动自检：移除硬编码旧三件套，完全动态探活并精准呈现「🟢 在线 / ⚪ 未启动 / 🔴 异常」三态语义；
  - 彻底清理设置页旧隧道设置与手工端口表：移除手动编辑表格，保存时自动物理剔除残留老旧配置字段；
  - 界面与监控深度联动：左导航统一为「SSH隧道管理」，右侧监控栏按卡片名称独立分类展示端口，系统托盘悬浮 Tooltip 丰富展示多行方案明细与状态；
  - 修复向导编辑态误报自身端口冲突与部署场景控件重叠，全套测试用例 511 例全过。

- ✅ **通用动态 SSH 隧道管理架构与向导助手（2026-09-02）**：
  - 彻底破除硬编码三卡片与固定拓扑限制，引入通用声明式 `TunnelItem` 数据模型（支持任意数量、任意正向/反向规则、多端口映射、断线常驻重连）；
  - 100% 向后兼容无感平滑迁移：系统自动从旧配置合成默认规则列表，老配置开箱即用；
  - 新增「隧道创建助手与场景向导（Tunnel Wizard）」：面向小白用户提供 4 大典型场景问答模板（在家中继正向/内网反向暴露/局域网直连/从已有部署生成）与高级自定义，内置本地端口占用冲突检测；
  - 重构 `ui/pages_tunnels.py`（动态卡片流 + 一键全部启动/停止 + 方案拓扑快照切换）；
  - 精简 `ui/pages_settings.py`（剥离冗余的旧隧道标量字段，收敛至核心连接与基础服务）；
  - 右侧健康监控栏（`RightBar`/`StatusPanel`）与系统托盘与动态隧道规则实时联动与热重载。
