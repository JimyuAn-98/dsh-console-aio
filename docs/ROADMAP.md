...
- ✅ **v0.7.0 发布**（2026-08-30, tag→Action 自动出安装包）+ CI 打包链补齐
  （pages_theme hidden-import / logo 资源 / exe 图标）
- ✅ 纯单元测试 349 例全过，3080 宿主无恙
...
1. **发版 v0.7.0**：提交当前批次（logo 全链路 + 主题色换版 + 实时主题页）→ CI 打包链
   ...
   安装包 → tag 发版
2. **P3 切入：诊断报告 + 配置导出/导入**：互为 feeds（报告含配置快照），是 P4 隧道
   规划器「一键诊断」的前置；复用 probe/env/deployments 基建；对小白价值最直接
   （出问题生成一份报告直接求助/贴 issue）
3. **用量图表**：数据端（token 按模型/天聚合）已就绪，纯展示层；强化"统计"域定位
4. **命令面板 Ctrl+K**：独立纯 UI 层，对键盘用户价值大、对小白一般，放后
5. **P4 两大件先写方案再动手**：隧道规划器 / 远程部署子工具组——涉及"开放受控远程
   写"的红线讨论（VISION §五.4 开放问题未拍板），单独立项
