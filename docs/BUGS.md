...
| BUG-013 | 全局包模式安装失败：`pnpm add -g` 报 `The configured global bin directory "...\pnpm\bin\bin" is not in PATH` | 待修（2026-09-11 用户实测） | 根因：`core/pkgmgr.py::pnpm_env` 把 `PNPM_HOME` 设成了全局 bin 目录本身，而 pnpm 会在 `PNPM_HOME` 后再拼 `bin`。修复方向：不再设置 `PNPM_HOME`，只把解析出的全局 bin 目录前置进 `PATH`（若环境已有 `PNPM_HOME`，则用 `<PNPM_HOME>\bin`）。本机三组对照：A `PNPM_HOME=<...\pnpm\bin>` 复现；B 仅 PATH 前置、C `PNPM_HOME=<...\pnpm>` 均正常。 |
| BUG-014 | 源码安装：目标目录已存在且非空但不是 dsh 仓库时仍跳过 clone，随后 git fetch 报 not a git repository | 待修（2026-09-11 用户实测） | core/env.py::install_dsh 的「目录已存在且有内容则跳过 clone」只看非空，未校验是否 dsh git 仓库，随后无条件按已 clone 处理。修复方向：跳过前校验 target/.git 与远程是否 dsh 仓库，否则中文报错提示换目录或先清空。日志 install-20260911-234503.log |
| BUG-015 | 版本 tag 前缀未规范化：dsh Release tag 形如 dsh-v0.1.5-rc.2，包模式 install_cmd 只去单个 v 前缀会拼出非法 npm 版本 @deepseek-ai/dsh@dsh-v0.1.5-rc.2 | 待修（2026-09-11 预判，已核实 tag 命名） | core/pkgmgr.py::install_cmd 与 core/dshctl.py::_deploy_pkg_version 仅 strip 单个 v/@。修复方向：统一版本规范化（dsh-v / v / 裸版本 -> npm 版本 0.1.5-rc.2），源码模式仍用原始 tag（git checkout 需要 dsh-v0.1.5-rc.2）。git tag 实测：dsh-v0.1.1..dsh-v0.1.5-rc.2 |
