...
| BUG-009 | 打包运行读不到 config.json：core/config.py 内**后置的旧 `_default_config_path` 定义覆盖了文件顶部新别名**（Python 自上而下执行），打包时 `__file__` 在临时解压目录 → 读写全落临时目录，源码运行不受影响（旧定义恰好在仓库根） | 已修复（2026-08-30） | 删除旧定义，load/save 直连公开 `default_config_path()`（frozen→exe 目录）；设置页显示配置实际路径；离屏复现+字节码比对定位 || BUG-013 | 全局包模式安装失败：`pnpm add -g` 报 `The configured global bin directory "...\pnpm\bin\bin" is not in PATH` | 待修（2026-09-11 用户实测） | 根因：`core/pkgmgr.py::pnpm_env` 把 `PNPM_HOME` 设成了全局 bin 目录本身，而 pnpm 会在 `PNPM_HOME` 后再拼 `bin`。修复方向：不再设置 `PNPM_HOME`，只把解析出的全局 bin 目录前置进 `PATH`（若环境已有 `PNPM_HOME`，则用 `<PNPM_HOME>\bin`）。本机三组对照：A `PNPM_HOME=<...\pnpm\bin>` 复现；B 仅 PATH 前置、C `PNPM_HOME=<...\pnpm>` 均正常。 |
